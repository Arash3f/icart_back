...
async def get_organization_additional_info(*,
                                           ...
                                           current_user:User = Depends(deps.get_current_user())):
    users_query = select(User).where(User.organization_id == current_user.self_organization_id)
    users = await user_crud.get_multi(db=db,query=users_query)
    users_id = [user.id for user in users]
    transaction_count = await db.execute(
        select(func.count()).
        select_from(Transaction).
        where(or_(Transaction.transferor_id.in_(users_id),Transaction.receiver_id.in_(users_id))).
        group_by(func.month(Transaction.created_at),func.year(Transaction.created_at)).
        subquery())
    return {"Users count": len(users_id),
